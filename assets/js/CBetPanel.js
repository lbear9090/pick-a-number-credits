function CBetPanel(){
    ...
    this._init = function(){
        ...
        */
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            ...
            _pStartPosAudio = {x: CANVAS_WIDTH - oSprite.width/2, y: (oSprite.height / 2) + 10};
        ...
        */
        ...
        if (EngagedNation.Config.Game.game_play_is_forecast_enabled) {
            _oArrowLeft = new CGfxButton(BET_PANEL_X + 8,(CANVAS_HEIGHT/2) + 50,s_oSpriteLibrary.getSprite("arrow_left"),_oContainer);
            _oArrowLeft.addEventListener(ON_MOUSE_UP,this._onArrowLeft,this);
            
            _oArrowRight = new CGfxButton(BET_PANEL_X + oSpriteBg.width -10,(CANVAS_HEIGHT/2) + 50,s_oSpriteLibrary.getSprite("arrow_right"),_oContainer);
    ...
    this._showCashOut = function() {
        _oChipPanel._showCashOut();
    }

    this._hideCashOut = function() {
        _oChipPanel._hideCashOut();
    }
    ...
    this.setSimpleBet = function(iIndex,iPlace,iFicheValue,oBut){
        if(iFicheValue > s_iCurMoney ){
            ...
            _oMsgBox.show(EngagedNation.Config.Game.text_not_enough_credits,true);
        ...
        if( (_iTotBet+iFicheValue) > MAX_BET){
            ...
            _oMsgBox.show(EngagedNation.Config.Game.text_bet_high,false);
    ...
    this.onStartExit = function(){
        ...
        if(_iTotBet < MIN_BET){
            _oMsgBox.show(EngagedNation.Config.Game.text_bet_low,false);
            return
        }
